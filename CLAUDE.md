...
### ✅ Session 10 — VAT-Inclusive OCR Fixed End-to-End (2026-06-05)

**OCR Architecture (final — ห้ามเปลี่ยนอีก):**
- Flow: Vision อ่าน image → raw Thai text → `classifyFromText()` detect doc_type + vat_inclusive → inject เข้า GPT prompt → GPT extract ทุก field
- **Vision หน้าที่**: อ่านตัวอักษรภาษาไทย + classify keywords เท่านั้น (ไม่ extract ตัวเลข)
- **GPT หน้าที่**: รับ text + pre-classified context → extract fields (GPT เป็น sole authority)
- Key files: `ocr/vision.go`, `ocr/gpt.go`, `ocr/service.go`, `ocr/crossverify.go`

**Bugs fixed this session:**
1. **GPT extract before_vat ผิด (vat_inclusive=true)** — เพิ่ม VISION HINT ใน GPT prompt ระบุ math hint เมื่อ Vision หา `VatInclusiveSubtotal` ได้ + เพิ่ม "(ไม่ใช่ มูลค่าที่มีภาษี)" ใน vatDesc → GPT เลือกถูก
2. **Vision regex window เล็กเกิน** — เพิ่มจาก 40 → 120 chars + เพิ่ม `vatInclSubtotalRegex` จับ "มูลค่าที่มีภาษี"
3. **crossVerify false positive** — Vision regex จับ total=2.08 (ผิด) ทำให้ status=conflict แก้: skip Vision total ถ้า < GPT total × 1%
4. **VerificationWizard Level 1 ผิด (vat_inclusive)** — เปลี่ยน compare `itemsSum` กับ `vat_inclusive_subtotal` แทน `total_before_vat` เมื่อ `vat_inclusive=true`
5. **ทศนิยมหายใน DB (root cause)** — `CASE WHEN $18 != 0 THEN $18` PostgreSQL infer `$18` เป็น integer เพราะ compare กับ `0` literal → 1945.79 → 1945 → เก็บ 1945.00 **แก้: เปลี่ยน total_before_vat/vat_amount/total_amount เป็น direct `= $n`** (ใน `UpdateInvoiceData`)

**OCR ทดสอบผ่านแล้ว:**
```
[ocr/classify] doc_type=receipt vat_inclusive=true
[ocr/gpt]      before_vat=1945.79 vat=136.21 total=2082.00 ✅
[worker]       status=verified matched=true ✅
```

### 🔑 กฎ DB: อย่า CASE WHEN กับ float columns
> `CASE WHEN $n != 0 THEN $n` — PostgreSQL infer type จาก integer literal `0` → cast param เป็น bigint → ตัดทศนิยม
> **กฎ**: financial amount columns ให้ใช้ direct `SET col = $n` เสมอ ไม่มี CASE WHEN

### 🟡 ถัดไป
- ทดสอบ OCR กับใบอื่น (doc_type=tax_invoice, vat_inclusive=false) ด้วย
- Phase: OneDrive API, PDF OCR, Password reset
