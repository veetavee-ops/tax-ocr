...
- **Session 13**: Dev labels, P-01-M unified form, OCR company extract
...
- `POST /api/v1/ocr/extract-company` — รับไฟล์รูป (JPG/PNG) → Vision → GPT → return CompanyData
- `ocr/company.go`: `CompanyData` + `BranchData` structs, prompt สำหรับ หนังสือรับรอง/ภพ.01/ภพ.20
...
- ⚠️ รองรับเฉพาะ JPG/PNG — PDF ต้องแปลงเป็น image ก่อน (Vision ไม่รับ raw PDF bytes)
- PDF digital (selectable text) → ทำได้ง่ายกว่าด้วย Go PDF library → Phase ถัดไป
...
- **PDF OCR company**: รับ digital PDF → extract text ด้วย Go PDF library → ส่ง GPT (ไม่ต้อง Vision)
