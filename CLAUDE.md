...
### ✅ Session 9 — OCR Architecture Final Fix + Bug Fixes (2026-06-05)

**OCR Architecture (final — ห้ามเปลี่ยนอีก):**
- Flow: Vision อ่าน image → raw Thai text → `classifyFromText()` detect doc_type + vat_inclusive ด้วย regex → inject classification เข้า GPT prompt → GPT extract ทุก field โดยรู้ context แล้ว
- **Vision หน้าที่**: อ่านตัวอักษรภาษาไทย + classify ด้วย keyword rules (ไม่ใช่ extraction)
- **GPT หน้าที่**: รับ text + pre-classified context → extract fields เท่านั้น (ไม่ classify เอง)
- **ลบ Vision override**: GPT เป็น sole authority สำหรับทุก field รวมตัวเลข (Vision regex ไม่น่าเชื่อถือพอ)
- Key files: `ocr/vision.go` (classifyFromText), `ocr/gpt.go` (extractFromTextWithContext), `ocr/service.go`

**classifyFromText() rules (vision.go):**
- `vat_inclusive=true` ถ้า text มี keyword: `มูลค่าที่มีภาษี|ราคารวมภาษี|รวมภาษีแล้ว`
- `doc_type`: ใบกำกับภาษี→tax_invoice, ใบเสร็จรับเงิน→receipt, ใบส่งสินค้า→delivery_note

**GPT context injection (gpt.go):**
- PRE-CLASSIFIED block ใน user message: ระบุ doc_type + vat_inclusive + คำอธิบายภาษาไทย/อังกฤษ
- ถ้า vat_inclusive=true: บอก GPT ให้ใช้ "มูลค่าสินค้าก่อนภาษี" จาก footer เป็น total_before_vat

**Bugs fixed this session:**
- `vendor_name NOT NULL` — `nullIfEmpty("")` ส่ง NULL ไป DB → แก้ใช้ `input.VendorName` โดยตรง
- `minio.go Upload` — เปลี่ยน `strings.NewReader(string(data))` → `bytes.NewReader(data)`

### 🐛 ปัญหาที่ยังต้องทดสอบ (Re-run OCR + ดู log)
```
[ocr/classify] doc_type=tax_invoice vat_inclusive=true   ← ต้องเป็น true สำหรับใบที่รวม VAT
[ocr/gpt]      before_vat=1945.79 vat=136.21 total=2082.00  ← ต้องถูกต้อง
```
- ถ้า `vat_inclusive=false` ทั้งที่ควรเป็น true → keyword "มูลค่าที่มีภาษี" ไม่อยู่ใน raw text → ดู `[ocr/vision/raw]`
- ถ้า before_vat/vat ยังผิด → ดู `[ocr/vision/raw]` แล้วแชร์ให้ดู

### 🔑 สำคัญมาก — สาเหตุที่ทศนิยมหายตลอด
> **ห้าม** ให้ Vision regex override ค่าจาก GPT อีก ถ้า Vision regex จับ 1945.00 (ตัดทศนิยม) แล้วไป override GPT ที่อ่านได้ 1945.79 = ผิด
> กฎ: GPT เป็น authority ด้านตัวเลขทุกตัว Vision แค่ classify + cross-verify เท่านั้น

### 🟡 ถัดไป
- ทดสอบ OCR ใบ VAT-inclusive ว่า classify ถูก + ตัวเลขถูกหรือเปล่า
- ถ้า GPT ยังตัดทศนิยม → ดู raw log + ปรับ prompt ให้ชัดขึ้น
