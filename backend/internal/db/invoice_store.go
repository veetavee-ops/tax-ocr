...
func (s *Store) CreateInvoice(ctx context.Context, input Invoice) (Invoice, error) {
	...
	err := scanInvoice(
		s.pool.QueryRow(ctx,
			...
			nullIfEmpty(input.VendorName), nullIfEmpty(input.VendorTaxID), nullIfEmpty(input.VendorAddress), nullIfEmpty(input.VendorBranchCode),
...
func (s *Store) FullUpdateInvoice(ctx context.Context, id string, u InvoiceUpdate) (Invoice, error) {
	...
	err := scanInvoice(
		s.pool.QueryRow(ctx,
			...
			nullIfEmpty(u.VendorName), nullIfEmpty(u.VendorTaxID), nullIfEmpty(u.VendorAddress), nullIfEmpty(u.VendorBranchCode),
